...
public class MouseLookCharacterController : MonoBehaviour {
	...
	void Update () {
		...
		//rotationX = Mathf.Repeat(rotationX,3);
		//rotationY = Mathf.Clamp(rotationY,-2, 85);
		
		Quaternion xQuaternion = Quaternion.AngleAxis (rotationX, Vector3.up);
		Quaternion yQuaternion = Quaternion.AngleAxis (rotationY, -Vector3.right);
		transform.Rotate(rotationY,rotationX,0);
		//transform.localRotation = originalRotation * xQuaternion * yQuaternion * Vector3.forward;
	}
}
*/